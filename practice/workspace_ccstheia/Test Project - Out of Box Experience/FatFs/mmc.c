...
/*-------------------------------------------------------------------------*/
//FatFs needs to use HAL functions for HW specfic read/write
static SDCardLib_Interface *sdInterface;

#define	INIT_PORT()     sdInterface->sdInit()       /* Initialize MMC control port */
#define FAST_MODE()     sdInterface->sdFastMode()   /* Maximize SD Card transfer speed */
#define DLY_US(f, n)    uint8_t i = (f/1000000);for(;i>0; i--){__delay_cycles(n);}  // Delay n microseconds           // KLQ

#define	CS_H()          sdInterface->sdSetCSHigh()  /* Set MMC CS "high" */
#define CS_L()          sdInterface->sdSetCSLow()   /* Set MMC CS "low" */
...
{
    sdInterface->sdSendFrame((uint8_t *)buff, bc);
}
...
)
{
    ...
    sdInterface->sdReadFrame(buff, bc);
}
